...
def train_epoch(model, dataloader, optimizer, scheduler, dihedral_constraints, device, config):
    """Train for one epoch with dihedral angle constraints"""
    model.train()
    total_loss = 0
    ...
    for batch_idx, batch in enumerate(progress_bar):
        ...
        combined_loss = mlm_loss + config.model.constraint_weight * total_dihedral_loss
    ...
    return avg_loss, avg_mlm_loss, avg_constraint_loss


def validate(model, dataloader, dihedral_constraints, device, config):
    """Validate the model with dihedral angle constraints"""
    model.eval()
    total_loss = 0
    constraint_loss_total = 0
    mlm_loss_total = 0
...
def main():
    ...
    train_dataset = EfficientProteinDataset(config.data.data_path, max_seq_len=config.training.max_seq_len)
    ...
    # Use a smaller validation set for faster validation
    ...
    for epoch in range(config.training.num_epochs):
        ...
        # Train
        train_loss, train_mlm_loss, train_constraint_loss = train_epoch(
            model, train_loader, optimizer, scheduler, dihedral_constraints, device, config
        )
        
        # Validate
        val_loss, val_mlm_loss, val_constraint_loss = validate(
        ...
        print(f"  Train Loss: {train_loss:.4f} (MLM: {train_mlm_loss:.4f}, Constraint: {train_constraint_loss:.4f})")
        print(f"  Val Loss: {val_loss:.4f} (MLM: {val_mlm_loss:.4f}, Constraint: {val_constraint_loss:.4f})")
