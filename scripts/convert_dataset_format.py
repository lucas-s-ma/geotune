...
def update_processed_dataset(dataset_path):
    ...
    with open(dataset_path, 'wb') as f:
        pickle.dump(updated_proteins, f)
    
    print(f"Updated and saved {len(updated_proteins)} proteins to new format")
    print("Dataset is now compatible with dihedral angle constraints!")
    
    # Verify the update
    sample_protein = updated_proteins[0] if updated_proteins else None
    if sample_protein:
