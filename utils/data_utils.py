        ...
        if self.include_structural_tokens:
            try:
                if idx < len(self.structural_tokens):
                    ...
                    # Ensure all structural tokens are in valid range [0, num_classes-1] or -100
                    # Convert any invalid tokens to -100 (ignore index)
                    validated_struct_seq = []
                    for token in struct_seq:
                        # Validate the token is in a reasonable range before truncating/padding
                        if not isinstance(token, (int, np.integer)) or token < 0 or token > 20:
                            validated_struct_seq.append(-100)  # Invalid token should be ignored
                        else:
                            validated_struct_seq.append(int(token))

                    # Truncate structural tokens to match sequence length
                    if len(validated_struct_seq) > self.max_seq_len:
                        validated_struct_seq = validated_struct_seq[:self.max_seq_len]

                    # Pad or truncate structural tokens to max length
                    if len(validated_struct_seq) < self.max_seq_len:
                        padding_length = self.max_seq_len - len(validated_struct_seq)
                        validated_struct_seq.extend([-100] * padding_length)  # Use -100 as ignore index for padding

                    result['structural_tokens'] = torch.tensor(validated_struct_seq, dtype=torch.long)
